...
## Final-blocker corrections (2026-09-05, independent review of 6655f24a / 9581516c)

Machine: `scutil --get ComputerName` → Trent's MacBook Pro; Trent's in-session request via the health twux brief. Scope: correctness of the lazy UI helpers/components/tests/browser matrix only; no cosmetic redesign, no server/core/scaffold edits. Byte splitting is kept (Trent explicitly chose completing it over deferring).

### What the review found and what changed

| Blocker | Correction |
| --- | --- |
| Delayed `role=status` sat inside the `aria-busy` reserve and was removed with it; AT may suppress the announcement. | `LazySurface` renders the busy reserve and the live region as **siblings** in a grid stack (same reserved layout). The `role=status` region mounts immediately, empty, and receives "Loading …" text after 200ms, so it is a live-region text change rather than an inserted node inside a busy subtree. `LoadFeedback` gained a `pending` prop (default true; styleguide demos unchanged). |
| Vite remembers every attempted preload for the page's lifetime, so a shared stylesheet (`Field-*.css`) that failed under Sections was silently skipped for Settings/Input, which became "ready" unstyled (`.field` display:block). | Stylesheet failure/load ownership is now a **shared registry** (`createLazyHost`): failures are witnessed once (via `vite:preloadError`, the thrown preload error, and a document scan for siblings Vite throws before reporting), repaired at most once at a time, and **every boundary gates readiness on the failed styles it needs** both before importing and after the module resolves. Resolved code is not readiness; required styles are. A boundary whose required style still fails is an error with a real retry, never unstyled content. |
| Retrying Sections also retried Objects' own permanently-503 stylesheet and was blocked by it. | Recovery is boundary-scoped: a stylesheet named after a registered lazy entry (`ObjectsView-*.css`) belongs to that boundary only; other asset stylesheets are shared dependencies. Sections' retry no longer requests or waits on Objects' stylesheet. |
| Failure provenance ignored query/initiator/attempt time and treated status 0 as a witnessed HTTP failure (favicon probe on the entry path, CSP `script-src 'none'`, cross-origin redirect all offered a fake retry). | Evidence must be about **this boundary's exact attempted URL** (a retry must report its own `?load-retry=N` URL; a first attempt the bare entry), started **during this attempt**, by a script/link initiator. Status ≥400 is a witnessed HTTP failure. **Status 0 is ambiguous** and is corroborated only by `navigator.onLine === false` or a `HEAD` probe of the same URL (never an import) that the network refuses; a probe that completes (any status, opaque redirect included) means the download was not a network failure and the operator gets safe reopen guidance. A delivered (2xx) entry behind a failed shared dependency is never retried. Only the declared entry is ever freshened; the shared runtime is never re-imported. |

Known, documented limits of the honest approach: the app has no runtime dependency graph, so an unrelated **shared** stylesheet that keeps failing would block sibling boundaries retryably (fail-closed, never unstyled) — the reviewer's unrelated-stylesheet case was an entry-owned stylesheet and passes. A transient status-0 failure that has healed by the time of the probe reads as "not a network failure" and gets safe reopen guidance rather than a retry; a CSP `connect-src` block of the probe would read as a network refusal. Neither can silently mark unstyled content ready or promise a retry Chrome has cached as rejected.

### Verification (all rerun after the corrections)

- Focused Vitest: **45/45** — `lazy-resource` 3, `lazy-component` 20, new `lazy-component.recovery` 10 (jsdom counterexample regressions: shared stylesheet repaired before a sibling is ready; sibling fails closed retryably while the shared style still fails; concurrent-attempt style failure gates readiness; boundary-scoped recovery; de-duplicated in-flight repair; unreported second stylesheet witnessed; icon-probe not treated as evidence; status-0 corroboration by probe/offline; HTTP failure needs no probe; pre-attempt download ignored), `LazySurface` 6 (live region outside busy subtree, present from mount), `SettingsModal` 6.
- Web typecheck: **0 errors, 0 warnings**. Production web build and `pnpm design-system` regeneration passed (same known size/circular-chunk warnings).
- Real production matrix `apps/web/src/lib/app/lazy-surfaces.browser.mjs` against the parent's disarmed loopback production server (`UI_SHOT_BASE=http://localhost:4411`, temp project data, output disabled): **16/16** — the 10 original scenarios plus 6 new counterexample contexts (cross-route shared CSS → Input styled with `display:flex` after a real re-request; unrelated Objects CSS 503 → Sections recovers, Objects CSS requested once; CSP → safe reopen, 0 script downloads, 1 probe; favicon probe on the entry path → safe reopen; CORS redirect → safe reopen; evaluation TypeError → safe reopen). Probes are `HEAD` requests, so every interception distinguishes them; interceptions model the network as down until the failure is visible. Expected browser network diagnostics are asserted per context (aborts, their refused probes, 503s, per-preload CSP refusals, the CORS diagnostic).
- Strict captures via the parent's Vite on 5411: `UI_SHOT_BASE=http://localhost:5411 pnpm ui-shot trigger-graph settings-input settings-system --strict` passed; inspected `.ui-shots/{trigger-graph,settings-input,settings-system}.png` — unchanged layouts, styled Input fields, no console errors.

Coverage limits are unchanged: system Chrome only (no WKWebView/Safari/Firefox), Controller surface not re-captured (mock fixture only), no hardware. The timing gain remains unproven; the 22.42% initial-byte reduction is the isolated earlier measurement and was not re-measured here.
