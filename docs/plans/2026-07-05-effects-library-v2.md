...
**Status:** COMPLETE — U1–U7 have landed and the close-out gates are green. This doc remains
the durable source of truth; the [status tracker](#status-tracker--resume-protocol) at the
bottom records the implemented slices. State anchors: `docs/reports/2026-07-05-elv2-u1-u4-state.md`,
`docs/reports/2026-07-05-elv2-u5-report.md`, `docs/reports/2026-07-05-elv2-u6-report.md`, and
`docs/reports/2026-07-05-elv2-u7-report.md`. The "Current state" section below predates U1-U4
and is stale — use the reports.
...
| U4 canvas engine | done | bf3b1fa, b498bf3, 6176708, 9665bce, 83c6a3e, 04958db | Full canvas engine in `packages/core/src/canvas/`: (M1) typed-play-node seam — playType/canvasScene through GraphNode→PlayAction→Voice, `canvas:<sceneId>` hosting via the EXISTING generatorId path, `playTypeForEffect` total mapping, web hydrate inference; (M2) field.ts refactored INTO canvas/sampler.ts, 12 UV textures re-anchored in-commit; (M3/M4) scene model + six pure element renderers, painter's-order alpha-over; (M5) four samplers as precomputed per-pixel tables (hoop grid/placements, strip arclength, cylinder region, footprint==planar-xz); (M6) `createCanvasSceneEffect` adapter + scene registry resolved inside `tryGetEffect` — compositor + bridge UNTOUCHED (no fork, locked dec 7); CANVAS_PARAM_SPEC (canvasRotDeg/OffsetX/Y/Scale, samplerRotDeg, speed, brightness, hue) drives via the standard modulation sweep (LFO-on-canvasRotDeg test); (M7) full lens chain polar/unpolar/log-polar/kaleido/mobius/tile/swirl + hyper4d world-space path; goldens: stripes+polar==rings, ordered chain composition; byte-determinism at the compositor seam (canvas + mixed canvas/hosted engine replays); perf 0.54ms/frame on the 548px kit (budget 5ms). Gates: typecheck clean; core 625 / web 1187 / server 227 / io 51. Scene presets + registry hookup for authored scenes → U5/U6. |
| U5 canvas UI | done | 8f13456 | Typed play palette (Add ▸ Play ▸ 7 collections); EffectGallery pre-filtered + LOCKED to `node.playType` with `store.pickEffect` guard as backstop; node-face type chip (`NodeCard.typeChip`); Canvas scene picker + generic `CANVAS_PARAM_SPEC` params in PlayNodeInspector; Objects ▸ Canvas Scenes JSON create/rename/duplicate/delete editor; show-doc scene persistence (`AuthoredState.canvasScenes` / `voice.Show.canvasScenes`) with core `setShow` registry sync so `canvas:<id>` renders in connected voice mode; ClipDoc graph/section/song scene closure + id remap. Report: `docs/reports/2026-07-05-elv2-u5-report.md`. Gates: typecheck clean; core 628 / io 51 / protocol 1 / server 227 / web 1217. ui-shot: typed play palette + node type chip captured; gallery-lock / canvas inspector / Objects JSON need rail+node selection the headless harness can't drive (pre-existing MasterDetail limitation) — covered by unit tests. |
| U6 library fill | done | PR #63 (+ review-fix commits) | 23 seed canvas scenes in core `canvas/presets.ts` (2 per lens family across all 8 lenses; `spiral-galaxy` chains log-polar→swirl), surfaced as gallery cards via `CANVAS_EFFECTS` in web fixtures + default presets; 4 gap-fill natives (`orbit-comet`, `scan-plane`, `drum-sonar`, `gravity-drops`) on the emission contract with tests (`u6-gap-fill.test.ts`). Review pass fixed: readonly seed typecheck error, registry count 45→49, gravity-drops hoop-gap dead zone, restored stripped fixtures.ts comments. Spec-alignment pass: built-ins core-only (D4, no seeding into show docs; `store.allCanvasScenes` = built-ins + authored, single card source), Objects "Built-in library" read-only group (duplicate to customise), gallery Lenses tab (D6, view of canvas pool — PlayType untouched), engine builtin re-register guard, FIXED pre-existing U5 buildShow structuredClone-on-proxy crash + hydrate strips persisted `canvas:*` effects. ui-shot: effect-gallery-canvas / effect-gallery-lenses captured clean (older -collection/-filtered shots broken since U5's lock → U7). Report: `docs/reports/2026-07-05-elv2-u6-report.md`. |
| U7 close-out | done | feat/u7-effects-library-closeout | GitHub issue #60 close-out implemented on top of PR #63. Full `pnpm ui-shot --all --strict` sweep green across 25 shots including Canvas, Lenses, collection, and filtered gallery states; live `:5173` spot-check authored and selected `U7 Spot Check 2026-07-05`, reloaded, and verified a lit Canvas node thumbnail (`1792` lit pixels). Gates green: targeted canvas determinism/perf (`2.469 ms/frame` on the 548px kit, budget 5ms), full non-desktop test sweep (core 633 / io 51 / protocol 1 / server 227 / web 1217), and `pnpm typecheck`. Report: `docs/reports/2026-07-05-elv2-u7-report.md`. |
