...
| U4 canvas engine | in-progress | — | M1 seam + M2 field→canvas refactor + M3/M4 scene model & element renderers done (canvas/types.ts scene/element/sampler/lens model; elements.ts pure renderers + painter's-order `sceneColorAt`) + M5 four samplers done (`buildSamplerTable` precomputed per-pixel placement: hoop auto-grid/placements, strip arclength line, cylinder region, footprint==planar-xz; 7 tests). M6 scene adapter done (`createCanvasSceneEffect` EffectGenerator adapter + scene registry resolving `canvas:<sceneId>` inside `tryGetEffect` — NO bridge/compositor edits; CANVAS_PARAM_SPEC 8 std params; LFO-on-canvasRotDeg modulation-sweep test; lenses.ts written, tests next). Next: lens tests + goldens + compositor-seam determinism + perf bench. |
