...
| U4 canvas engine | in-progress | — | M1 typed-play-node seam done (GraphNode/PlayAction/Voice carry playType+canvasScene; pool hosts canvas scenes as `canvas:<sceneId>` generator ids; `playTypeForEffect` total mapping; web hydrate inference pass). Next: field.ts → canvas module refactor. |
