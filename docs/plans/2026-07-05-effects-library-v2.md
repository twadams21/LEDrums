...
| U4 canvas engine | in-progress | — | M1 seam + M2 field→canvas refactor + M3/M4 scene model & element renderers done (canvas/types.ts scene/element/sampler/lens model; elements.ts pure renderers + painter's-order `sceneColorAt`). Next: four samplers. |
