...
- **`--radius-card` — KEEP (Trent, decided):** leave the token in place (value `0px`, square) so a future
  radius can be reintroduced in one spot. Just **tighten the comment** to say it's intentionally retained as
  the single card-corner knob (flip to a non-zero value to soften the whole shell). Do NOT remove it or its
  ~24 `var(--radius-card)` uses — that's the point of the token.
...
## Acceptance
Stale comments corrected; one shared dark-pixel RGB constant; `--radius-card` kept + its comment tightened
(per Trent — retained for future radius); full sweep green; no behaviour change.
