...
- **MIDI Clock** (Settings › Input › Timing source): the transport's tempo, start, continue and stop can follow an external MIDI beat clock instead of the authored tempo. Manual is the default, so an unsolicited clock never takes over a show. In MIDI Clock mode pick ONE clock input — the desktop app's native `LEDrums` port or one WebMIDI input of the editor's browser — then in Ableton turn on **Sync** for that MIDI Output. The status line shows waiting / running / stopped / lost and the measured tempo (marked *seed* until enough pulses have locked it). No pulse for 1s means *lost*: the transport freezes and stops until the clock returns, or you switch back to Manual (which resumes on the authored tempo — use the "Use … bpm" button first to keep the clock's tempo). MIDI Clock synchronises the **transport**; effects and LFOs authored in seconds keep running as authored, only beat-based timing follows the clock.
