...
export interface ShowIntegrityInput {
  ...
  /** Graph map keys ("drumId:zone") — each must resolve to a kit drum. */
...
/**
 * Assert the web's authored Show resolves: every graph key's drum exists in the kit,
 * and every setlist slot reference points at a real graph (slots reference graphs;
 * graphs reference drums). Pure + kit-agnostic — the caller supplies the kit's drum
 * ids — so it lives in core and the web boundary reuses it. Throws
