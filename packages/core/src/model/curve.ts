...
/**
 * The pass-through curve: out = in. The value an absent curve stands in for (S8's velocity
 * transfer curve, whose "no curve authored" state must be exactly today's behaviour).
 * Contrast {@link DEFAULT_CURVE}, the falling shape a decay ENVELOPE opens at.
 */
...
/**
 * Whether this curve is the pass-through. Checked on the NORMALISED value and by SHAPE
 * rather than by field equality: with the bipolar fader (F4) a strength of exactly 0 is a
 * dead-straight line under either bendable profile, so `{sCurve, strength: 0}` is the same
 * curve as `{bend, strength: 0}` and both count. `snap` never does — a step is a shape.
 * Storing a curve that changes nothing is noise, which is what this predicate exists to spot.
 */
