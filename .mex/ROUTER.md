...
- **Open follow-ups (pick with Trent):** routing is **live in voice mode + browser-refresh-safe** but **not persisted to disk across a server restart** — a "Save project" action (wired to the existing `saveProject` WS message; absent from the unified shell) is unbuilt. **Live `:5173` spot-check still owed** (no slice drove a browser): rewire-reroutes-in-voice-mode, drum geometry + literal pixel count, MIDI/OSC trigger map, controller Art-Net/sACN incl. priority/port/iface, node read-outs + renames-on-face.
