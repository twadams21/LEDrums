...
- **Follow-ups (not blocking; pick with Trent):** (a) wire `store.patchLabels` into `PatchNode` display so renames show on the graph node, not only the Inspector; (b) dataline read-out maps by transmit index — exact on (re)entry, can drift after an in-session rewire without remount; (c) sACN **priority** + per-output `port`/`iface` not plumbed through `setOutput` (needs a small core+server+protocol slice). **Live `:5173` spot-check still owed** (no slice drove a browser): rewire-reroutes-in-voice-mode, drum geometry + literal pixel count, MIDI/OSC trigger map, controller Art-Net/sACN, node read-outs.
