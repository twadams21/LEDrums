...
- **V in flight** (`v-velocity-fold`): fold `on:'velocity'`→`value`+`bands` (even cutoffs + per-band `fromPort` re-wire, behaviour-preserving; idempotent hydrate migration); drop `velocity` from `SwitchOn` (web+core) + Inspector selector. `section`/`beat` stay.
