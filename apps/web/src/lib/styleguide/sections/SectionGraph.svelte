...
<section class="block" id="graph">
  ...
  <div class="contract">
    ...
    <ul>
      ...
      <li><strong>Reconnect dots reveal on wire hover.</strong> The grab dots at a wire's ends stay hidden at rest (zero noise in a dense graph) and appear — instantly — while their wire is hovered or selected (<code>WireEdge</code> pushes the state into xyflow's label portal). Source end is a hollow ring, target end filled, so direction reads from the pair; entering the 25px grab range steps the dot up to 13px accent, lights the owning wire, and mid-drag the old run dims while the connection line previews the new one.</li>
