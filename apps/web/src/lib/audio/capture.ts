...
export class AudioCaptureSession {
  ...
  /** Resources acquired before permission/resume settles still belong to Stop. */
  ...
  async start(deviceId: string | null): Promise<void> {
    ...
    try {
      ...
      this.cancelStartup = null; // ownership has moved to the live session
      this.report({
      ...
      // An older permission request must never clear a newer start's cleanup handle.
      if (this.cancelStartup === abandon) this.cancelStartup = null;
    }
  }
